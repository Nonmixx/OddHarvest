          ...
          <div className="flex flex-col md:flex-row gap-6">
            ...
            <div className="flex-1 space-y-3">
              <div className="flex flex-wrap items-center gap-3">
                <h1 className="text-2xl font-heading font-bold text-foreground">{seller.farmName || seller.name}</h1>
              ...
              <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
                <span className="flex items-center gap-1"><MapPin className="h-4 w-4" /> {seller.location}, {seller.state}</span>
                {seller.yearsExperience && (
                  <span className="flex items-center gap-1"><Calendar className="h-4 w-4" /> {seller.yearsExperience} {t("seller.years_exp")}</span>
                )}
          ...
          {reviews.map((review) => (
            <div key={review.id} className="farm-card p-4 space-y-2">
              ...
              <div className="flex gap-0.5">
                {[1, 2, 3, 4, 5].map((star) => (
                  <Star key={star} className={`h-4 w-4 ${star <= review.rating ? "text-farm-orange fill-current" : "text-muted-foreground"}`} />
                ))}
              </div>
